...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (type === 'session') {
      ...
      const html = wrapEmail(`<h2 style="font-size:20px;font-weight:900;margin:0 0 4px;color:#0d0d0d;">Violation Payment Receipt</h2><p style="margin:0 0 20px;font-size:13px;color:#888;">${v.status==='paid'?'Thank you for your payment.':'This violation is currently unpaid.'}</p><table width="100%" cellpadding="10" cellspacing="0" style="background:#f5f5f5;border-radius:8px;margin-bottom:20px;"><tr><td style="color:#888;font-size:13px;border-bottom:1px solid #e0e0e0">Violation ID</td><td style="font-weight:700;border-bottom:1px solid #e0e0e0">${v.id}</td></tr><tr><td style="color:#888;font-size:13px;border-bottom:1px solid #e0e0e0">Plate</td><td style="font-weight:700;border-bottom:1px solid #e0e0e0">${v.plate}</td></tr><tr><td style="color:#888;font-size:13px;border-bottom:1px solid #e0e0e0">Location</td><td style="font-weight:700;border-bottom:1px solid #e0e0e0">${v.lot_name||'—'}</td></tr><tr><td style="color:#888;font-size:13px;border-bottom:1px solid #e0e0e0">Violation</td><td style="font-weight:700;border-bottom:1px solid #e0e0e0">${v.violation_name||'—'}</td></tr>${v.parking_fee>0?`<tr><td style="color:#888;font-size:13px;border-bottom:1px solid #e0e0e0">Unpaid Parking Fee</td><td style="font-weight:700;border-bottom:1px solid #e0e0e0">$${v.parking_fee.toFixed(2)}</td></tr><tr><td style="color:#888;font-size:13px;border-bottom:1px solid #e0e0e0">Violation Fine</td><td style="font-weight:700;border-bottom:1px solid #e0e0e0">$${(v.fine_amount-v.parking_fee).toFixed(2)}</td></tr>`:''}<tr><td style="color:#888;font-size:13px;">${v.status==='paid'?'Amount Paid':'Amount Due'}</td><td style="font-weight:900;font-size:18px;color:${v.status==='paid'?'#2e7d32':'#d32f2f'};">$${v.fine_amount.toFixed(2)}</td></tr></table><p style="margin:0;font-size:13px;color:#444;">Need to pay a violation? <a href="https://www.cityparkmanagement.app/payfine" style="color:#2563eb;font-weight:700;">Pay here</a></p>`);
